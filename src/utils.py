...
def get_paths(folders: dict, input_dir: str, output_dir: str) -> dict:
    ...
    folders_with_paths = {
        ...
        **{key: os.path.join(input_dir, f'{value}/') for key, value in folders.get('input', {}).items()},
        # Проходим по всем директориям в 'output_dir' и добавляем базовый путь 'output_dir'
        **{key: os.path.join(output_dir, f'{value}/') for key, value in folders.get('output', {}).items()}
    }
